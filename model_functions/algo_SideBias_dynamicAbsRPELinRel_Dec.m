function stats_sim = algo_SideBias_dynamicAbsRPELinRel_Dec(stats_sim,xpar)
% % funDQ_RPE % 
...
aUnrew      = xpar(4);
decay_rate  = xpar(5);
updateRate  = xpar(6);   % fit update rate for omegaV
omega0      = xpar(7);       % initial omegaV at trial 1 (indicating stimulus-based bias)
...
if stats_sim.currTrial == 1  %if this is the first trial, initialize
    ...
    DV_left = vS_left*stats_sim.omegaV(T) + stats_sim.qL(T)*(1-stats_sim.omegaV(T));
    DV_right = vS_right*stats_sim.omegaV(T) + stats_sim.qR(T)*(1-stats_sim.omegaV(T));
