function [negloglike, nlls, pl, V_hist, omegaVs, RPE, Rel_diff] = funSideBias_dynamicAbsDeltaVLinRelDec_SubjectFixedRho(xpar,dat, initVals)
% % funDQ_RPE % 
...
omega0      = xpar(7);   % initial omegaV at trial 1
decay_omega = xpar(8);

if exist('initVals','var')
    if isfield(initVals,'Rho')
        Rho     = initVals.Rho;        % betaStim = betaDV*R, betaAct = betaDV*(1-R)
    else
        error("No \rho parameter input found");
    end    
    if isfield('initVals','omega0')
        omega0  = initVals.omega0;    % if using input from previous block
    end
else
    error("No initial parameter assigned");
end
...
%% looping through trials
    ...
    deltaRel = abs(V.Cir-V.Sqr) - abs(V.Left-V.Right);   % ranges [-1, 1], positive if Stim more reliable
    if deltaRel>0
