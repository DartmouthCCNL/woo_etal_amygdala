...
%       (metric_name + _ + decomp_map values)
    ...
    for x_num = 1:length(x_unique)
        ...
        for y_num = 1:length(y_unique)
            ...
            x_entropy = nansum_zero_helper([x_entropy, prob_x_given_y*prob_x*log2(prob_x_given_y)], 'omitnan');
        end
        if decompose_flag
    ...
    output.(metric_name) = nansum_zero_helper(x_entropies_storage, 'omitnan');
