...
% Inset data: stats obtained from Table S3.11-14
BarDat = struct;
SE_Dat  = struct;
BarDat.ERDS_Stim = [-0.076933, -0.033402 , -0.62615]; % control, amyg, VS
SE_Dat.ERDS_Stim = [0.045091, 0.047047, 0.06612];
BarDat.ERDS_Act = [-0.019254, 0.013757 , 0.191783 ]; % control, amyg, VS
SE_Dat.ERDS_Act = [0.042687, 0.042858, 0.053587];
BarDat.medianRT = [6.4037, 1.0906, 30.662 ]; % control, amyg, VS
SE_Dat.medianRT = [4.7777, 4.833257, 6.381877];
BarDat.Omega_0 = [-0.11273, 0.024298, 0.4552 ]; % control, amyg, VS
SE_Dat.Omega_0 = [0.061507, 0.058171, 0.12367];
plot_fields = fieldnames(BarDat);
...
for m = 1:length(MET_set)
    ...
    for d = 1
        ...
        % bar plots
        for g = 1:numel(groups.labels)
            bar(g, BarDat.(plot_fields{m})(g),'FaceColor',groups.colors{g},'EdgeColor',groups.colors{g}); hold on;
            errorbar(g, BarDat.(plot_fields{m})(g), SE_Dat.(plot_fields{m})(g),'LineStyle','none','Marker','none','Color',ones(1,3).*double(g==1)*0.5);
        end
        ...
        title("\beta_{sess}",'FontWeight','normal');
