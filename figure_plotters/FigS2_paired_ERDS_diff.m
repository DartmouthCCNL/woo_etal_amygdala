    ...
    for b = 1:numel(task_subsets) 
        ...
        for s = 1:numel(schedules.subsets)
            ...
            for g = 1:numel(groups.labels)
                ...
                % t-test from zero
                H = ttest(dataToPlot,0.0,'Alpha',0.001);
                ...
                text(mu*1.1+0.075*sign(mu), pdf(pd, mu)*1.12, num2str(cohens_d,3)+astT, 'FontSize', 12, 'Color',groups.colors{g},'HorizontalAlignment','center');          
             end
