...
if exist(output_dat,'file')
    ...
    disp("Output file loaded: "+output_dat);
else
    pBetterStruct = struct;
    %% Original code used to compute P(Bettter) across groups and save output
...
for pp = 1:3
    ...
    for g = 1:length(groups.labels)
        subject_set = fieldnames(pBetterStruct.(dataset_label).(groups.labels(g)).(block_type));
        subject_set = setdiff(subject_set, 'allSub');
        Sub_Mean = nan(length(subject_set), 80); % 80 trials per block
        for sub = 1:length(subject_set)
            Sub_Mean(sub,:) = pBetterStruct.(dataset_label).(groups.labels(g)).(block_type).(subject_set{sub}).stochastic.mean;
        end
        Xmean = mean(Sub_Mean, 1);
        Xsem  = sem(Sub_Mean, 1);
