...
for dataset_label = ["WhatWhere","Costa16"]
    ...
    [D, GroupDat, SubDat] = compile_param_data(M, block_idx, modNum, dataset_label);

    Mod1 = M.(dataset_label).(groups.labels(1)){modNum};
    for p = 1:length(GroupDat)    
        nexttile;
...
function [D, GroupDat, SubDat] = compile_param_data(M, block_idx, modNum, dataset_label)
    ...
    for g = groups_to_plot
        ...
        sub_ids = block_idx.(dataset_label).(group_label).animal_id;  
        switch dataset_label
            case 'Costa16'  
        ...
        for p = 1:length(Mod1.plabels)        
            ...
            for sj = 1:length(subject_set)
                ...
                SubDat{p}.(group_label)(sj) = mean(params(sub_idx,p),'omitnan');
            end
        end
